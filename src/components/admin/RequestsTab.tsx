     ...
     return (
          <div className="space-y-4">
               <div className="flex items-center space-x-2 mb-6 bg-white p-2 rounded-xl border border-stone-200 inline-flex">
                    ...
                    <select value={filter} onChange={e => onFilterChange(e.target.value as any)} className="bg-transparent border-none text-sm font-medium text-stone-700 focus:ring-0 cursor-pointer">
               ...
               {filtered.length === 0 ? (
                    <div className="bg-white p-8 rounded-2xl text-center border border-stone-200 text-stone-500">Nenhuma solicitação encontrada para este filtro.</div>
               ) : (
                    filtered.map(req => (
                         <div key={req.id} className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200 flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                              ...
                              <div className="flex flex-wrap items-center gap-2 shrink-0">
                                   {req.status === 'pending' && (<>
                                        <button onClick={() => handleUpdateStatus(req.id!, 'confirmed')} className="flex items-center px-4 py-2 bg-stone-900 text-white rounded-xl hover:bg-stone-800 transition-colors text-sm font-medium"><CheckCircle2 className="w-4 h-4 mr-2" />Aprovar</button>
                                        ...
                                        <button onClick={() => handleUpdateStatus(req.id!, 'rejected')} className="flex items-center px-4 py-2 bg-stone-100 text-stone-700 rounded-xl hover:bg-stone-200 transition-colors text-sm font-medium"><XCircle className="w-4 h-4 mr-2" />Recusar</button>
                                   </>)}
