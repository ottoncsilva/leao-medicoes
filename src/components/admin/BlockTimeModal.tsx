          ...
          <div className="fixed inset-0 bg-stone-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
               <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
                    ...
                    {slot ? (
                         ...
                         <p className="text-sm text-stone-500 mb-4">Informe o período que deseja bloquear manualmente.</p>
                    )}
                    <form onSubmit={handleSubmit}>
                         <div className="mb-4">
                              ...
                              <input
                                   ...
                                   className="w-full px-3 py-3 border border-stone-300 rounded-xl focus:ring-stone-900 focus:border-stone-900 sm:text-sm"
                              ...
                              <>
                                   <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                             ...
                                             <input type="date" required value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-3 py-2 border border-stone-300 rounded-xl focus:ring-stone-900 focus:border-stone-900 sm:text-sm" />
                                        </div>
                                        <div>
                                             <label className="block text-sm font-medium text-stone-700 mb-2">Hora Início</label>
                                             <input type="time" required value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-3 py-2 border border-stone-300 rounded-xl focus:ring-stone-900 focus:border-stone-900 sm:text-sm" />
                                        </div>
                                   </div>
                                   <div className="grid grid-cols-2 gap-4 mb-6">
                                        <div>
                                             <label className="block text-sm font-medium text-stone-700 mb-2">Data Fim</label>
                                             <input type="date" required value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-3 py-2 border border-stone-300 rounded-xl focus:ring-stone-900 focus:border-stone-900 sm:text-sm" />
                                        </div>
                                        <div>
                                             <label className="block text-sm font-medium text-stone-700 mb-2">Hora Fim</label>
                                             <input type="time" required value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-3 py-2 border border-stone-300 rounded-xl focus:ring-stone-900 focus:border-stone-900 sm:text-sm" />
                                        </div>
                                   </div>
                              </>
                         )}
                         <div className="flex justify-end space-x-3 mt-6">
                              ...
                              <button type="submit" disabled={isLoading} className="px-4 py-2 bg-stone-900 text-white rounded-xl hover:bg-stone-800 transition-colors text-sm font-medium disabled:opacity-50">
